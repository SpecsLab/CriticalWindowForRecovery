...
%   vector for a vector input. W specifies the size of the window and
...
try
    ...
    % feval did ok. Now check for vector output
    if size(R,2) ~= 1 || length(size(R))~=2
        error('Function "%s" does not return a vector output for a vector input.', FUNstr) ;
    end
...
R = R(ones(size(V,1),size(R,1))) ; % pre-allocation !!
